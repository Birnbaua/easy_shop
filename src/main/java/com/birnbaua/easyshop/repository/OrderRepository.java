...
public interface OrderRepository extends JpaRepository<Order,OrderId> {
	...
	@Query(value = "SELECT o FROM Order o WHERE o.table.shop=?1 AND o.isOpen = true")
	public Page<Order> findOpenOrders(String shop, Pageable pageable);
	
	@Query(value = "SELECT o FROM Order o WHERE o.table.shop=?1 AND o.updatedAt < ?2 AND o.isOpen = true")
	public Page<Order> findOpenOrders(String shop, Timestamp from, Pageable pageable);
	
	@Query("SELECT o FROM Order o WHERE o.table=?1 AND o.isOpen = true")
	public List<Order> findOpenOrders(ShopTable table);
	
	@Query(value = "SELECT o FROM Order o WHERE o.table=?1 AND o.isOpen = true ORDER BY o.updatedAt ASC")
	public Page<Order> findOpenOrders(ShopTable table, Pageable pageable);
	
	@Query(value = "SELECT o FROM Order o WHERE o.table.shop=?1 AND o.updatedAt < ?2 AND o.isOpen = true")
	public Page<Order> findOpenOrders(ShopTable table, Timestamp from, Pageable pageable);
	...
	@Query("SELECT o FROM Order o Where o.table.shop=?1")
	public Page<Order> findOrders(String shop, Pageable pageable);
